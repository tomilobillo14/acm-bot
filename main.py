...
# Estado e información de cada usuario en memoria
states = {}
data   = {}
...
def kb_si_no(p):
    ...
    m.row(InlineKeyboardButton("✅ Sí", callback_data=f"{p}_si"),
...
def cmd_cancelar(msg):
    clear(uid(msg))
    bot.reply_to(msg, "❌ Cancelado. Enviá /nuevo\\_acm para empezar de nuevo.")

@bot.message_handler(commands=['generar'])
def cmd_generar(msg):
    u = uid(msg)
    if get_state(u) != 'LINKS':
        bot.reply_to(msg, "⚠️ Primero iniciá un ACM con /nuevo\\_acm")
        return
    d = get_data(u)
    links = d.get('links', [])
    if not links:
        bot.reply_to(msg, "⚠️ No hay links cargados. Pegá al menos uno.")
        return
    ...
    if not comparables:
        bot.edit_message_text(
            "❌ No pude extraer datos. Verificá que los links sean avisos individuales.",
...
def handle_text(msg):
    ...
    if state == 'DIR':
        ...
        try:
            ...
            bot.reply_to(msg, "⚠️ Solo el número. Ej: *1*")
        ...
        try:
            ...
            bot.reply_to(msg, "⚠️ Solo el número. Ej: *30*")

    elif state == 'SCUB':
        try:
            set_val(u, 'sup_cub', float(text.replace(',', '.')))
            set_state(u, 'SSEMI')
            bot.reply_to(msg, "Superficie *semi cubierta* (m²) — escribí *0* si no tiene:")
        except ValueError:
            bot.reply_to(msg, "⚠️ Solo el número. Ej: *41.08*")
        ...
        set_state(u, 'SDESC')
        bot.reply_to(msg, "Superficie *descubierta/patio* (m²) — escribí *0* si no tiene:")
        ...
        try:
            ...
            set_val(u, 'cochera', 0)
        set_state(u, 'DIAS')
        bot.reply_to(msg, "⏳ *8/8 — Días en el mercado*\n_(0 si aún no está publicada)_")
        ...
        try:
            ...
            set_val(u, 'dias', 0)
        set_state(u, 'UB')
        bot.reply_to(msg, "📍 *Coeficiente de Ubicación (Ub.)*\n"
...
def cb_asc(call):
    ...
    bot.edit_message_text(
        ...
        u, call.message.message_id, parse_mode='Markdown')
    set_state(u, 'ANTIG')
...
def cb_ub(call):
    ...
    bot.edit_message_text(
        ...
        f"📐 Cub: {d.get('sup_cub',0)}m² | Semi: {d.get('sup_semi',0)}m² | Desc: {d.get('sup_desc',0)}m²\n"
        f"   _Homolog: {sup_h} m²_\n"
        f"📅 {d.get('antiguedad','?')} años | {d.get('dias',0)} días\n"
        f"🔧 {d.get('estado_label','')}\n"
        f"📊 Coef estimado: *{ct}*\n\n"
        "─────────────────────────\n"
